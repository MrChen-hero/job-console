...
describe('library store（双通道合并）', () => {
  ...
  it('分类图标在改名、重新加载、备份导入后保留，兼容没有图标的旧数据', async () => {
    ...
    await store.renameCategory('复盘', '我的复盘', 'star')
    ...
    await store.load()
    expect(store.categoryRows.find((r) => r.name === '我的复盘')?.icon).toBe('star')
  ...
  it('已有分类可只改图标，内置分类改回原名仍保留图标', async () => {
    const store = useLibraryStore()
    await store.load()
    await store.addCategory('复盘', 'history')
    expect(await store.renameCategory('复盘', '复盘', 'target')).toBe(true)
    expect(await store.renameCategory('八股面经', '八股面经', 'star')).toBe(true)
    await store.renameCategory('八股面经', '基础知识')
    await store.renameCategory('基础知识', '八股面经')
    await store.load()
    expect(store.categoryRows.find((r) => r.name === '复盘')?.icon).toBe('target')
    expect(store.categoryRows.find((r) => r.name === '八股面经')?.icon).toBe('star')
