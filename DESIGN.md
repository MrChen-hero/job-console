...
| ≤1180 | 简历页编辑/预览切换，顶部保留版本、基本资料保存状态与导出；A4 同步缩放宽高 |
| ≤720 | KPI 紧凑两列并隐藏迷你图，待办优先于漏斗；投递卡片列表与折叠筛选；演示站单列，磁贴保持 2 列 |

## 交互细则

- 待办按日期升序，以日期块区分已逾期（危险色）、今天（主色）、后续（中性色），不显示分组标题；默认展示全部，列表最高 360px，溢出滚动，滚动条在悬停或键盘焦点进入时显示。零变化用中性文字“暂无变化”。
- 材料库搜索、新建、上传和分类管理位于顶部工具栏，分类条只负责选择；分类管理按钮至少 40px 高。摘要使用 Markdown 解析后的纯文本。
- 手机导航打开时主内容 inert，焦点进入导航；Tab 限制在导航内，关闭后返回触发按钮。弹层必须在导航与主内容的 inert 容器之外。
- 导入以独立弹窗展示当前/备份数量，选择方式后执行，取消不产生写入。覆盖需明确确认；保存失败保留输入并显示错误。
- 项目卡片显示演示数量，内部操作通过焦点或悬停显现；卡片的 Enter/Space 只处理自身，不干扰编辑和删除按钮。
