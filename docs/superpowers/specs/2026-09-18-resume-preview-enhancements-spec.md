...
**首次打印记录的边界**：原临时测试记录“预览 3 页 = PDF 3 页”，但未覆盖打印样式稳定后测量层被隐藏导致的零高度重测；该记录不能单独支持 Checkpoint 3 验收。

**2026-09-18 重新打开并验收 Checkpoint 2 / 3**：
- 标题与首条内容整体放置；组合超高时独占一页并允许溢出，连续标题也纳入组合，不额外制造标题页。纯函数新增超高组合、连续标题及恰好填满的回归测试。
- 测量层改为 Teleport 到 body 的零尺寸固定裁切容器，保持不可见但持续参与布局；窄屏编辑态与打印态都可测量。零尺寸通知不覆盖已有有效测量。
- 持久测试 `tests/e2e/resume-layout.spec.ts` 在桌面、手机、平板三个 Chromium 视口运行：等待打印布局跨过尺寸观察器及 Vue 更新后，比较逐页内容分配；实际导出 PDF 比较物理页数；返回屏幕后再次比较；验证编辑态直接打印与保存后打印。
- 本次 6 项 E2E 全部通过，退出码 0；305 项单测、lint、build 退出码均为 0。PDF 作为测试附件留在测试输出目录。超高单块/组合不属于物理页数一致的承诺范围。

**遗留**：Chromium 以外的浏览器（Firefox / Safari）打印分页未实测。`break-after: page` 与 `@page size` 均为标准属性，但各引擎对 `height:auto` 纸面的舍入处理可能不同。


**后续布局优化**：纸面几何和上传交互已由 `2026-09-18-resume-layout-refinement-spec.md` 的用户确认方案更新：上下 14mm / 左右 15mm、12px 正文、空区块不输出标题，头像支持点击/拖放。最终验收记录以该后续规格为准，本文 18mm/17mm 为首次设计记录。
