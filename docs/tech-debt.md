...
| D-7 | 简历分页按块切分，超高块会溢出到下一张纸 | 已知限制 |
| D-8 | 头像存缩放后原图，随版本复制放大备份体积 | 已知限制 |
...
2026-09-18 起简历预览为真分页：`sheet/blocks.ts` 把资料池展开成一维块序列（条目级粒度），`sheet/paginate.ts` 按实测高度装箱，标题块与其后第一块绑定同页。分页**只发生在块之间**，不做块内切分。

**现状**：单个块或「标题 + 首条内容」组合高于一页（当前内容高度 269mm）时独占一页并溢出，浏览器打印可能分成多张物理纸；这种情况下不保证预览逻辑页数与物理纸张数一致。组合整体放置，不额外产生只有标题的页。实际阈值由字体和换行决定，不能简单以条目行数保证不会出现。

**为什么这么定**：从中间截断一组 bullets 比跳页更难读，而块内切分需要引入「续页标题 + 续接标记」一整套排版语义。真出现超高条目，正确做法是拆成两条而不是让分页器截断。

## D-8 头像存缩放后原图，随版本复制放大备份体积（已知限制）

头像按「原图 + 归一化裁剪框」存（`BasicInfo.avatar` / `avatarCrop`），换来随时重裁、不必重新选图，也避免「参数改了成品没跟上」的不一致。上传时用 canvas 把长边压到 1400px、转 JPEG 0.85，约 150–300KB。

**代价**：资料池按版本独立，新建/复制版本会连原图一并复制（见 AGENTS.md §存储层），5 个版本约 1–1.5MB，导出备份因 base64 再涨约 1/3。

**若要收口**：可改成按内容哈希存一份头像表、各版本只存引用，需新增 Dexie 表并升备份版本号——代价是旧备份导入路径要处理缺表，暂不值得。

## 已收口

- **2026-09-18 编辑体验优化** 基本信息分组布局；头像弹窗支持点击/拖放上传、缩放平移还原和恢复居中，取消时使旧异步读取失效。纸面改为 14mm/15mm 页边距、12px 正文和紧凑间距，完整内置示例含头像在 Chromium 三视口均为一页 PDF；长内容继续分页。空区块不占标题空间，数据与编排仍保留。

- **2026-09-18 复核修复** 打印时隐藏测量层会触发零高度重测，把多页合并成长纸。测量层现挂到 body 的零尺寸裁切容器，编辑/打印均保留布局；新增 `tests/e2e/resume-layout.spec.ts`，三个 Chromium 视口验证稳定打印状态的逐页内容、PDF 页数、返回预览以及编辑态直接打印/保存后打印。标题与首条合计超高时改为整体放置，限制见 D-7。
- **2026-09-18** 简历预览改为真分页并补上区块标题编辑与一寸证件照头像（spec：`docs/superpowers/specs/2026-09-18-resume-preview-enhancements-spec.md`）。`@page margin:0` 消除重复页边距；首次临时验证的“3 页 = 3 页”未覆盖打印稳定后的零测量问题，已由上述持久回归用例补齐。三个新字段（`avatar`/`avatarCrop`/`subtitle`）均为可选，`BACKUP_SCHEMA_VERSION` 与 Dexie schema 都不变，旧 v4 备份照常导入。
