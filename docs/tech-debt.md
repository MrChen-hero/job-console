...
**现状**：单个块高于一页（261mm）时独占一页并溢出，浏览器打印会自然溢到下一张物理纸。实际要达到这个高度，一个项目/实习条目需要约 49 行要点，正常简历不会出现。
...
- **2026-09-18** 简历预览改为真分页并补上区块标题编辑与一寸证件照头像（spec：`docs/superpowers/specs/2026-09-18-resume-preview-enhancements-spec.md`）。打印超页的根因是 `@page margin:12mm` 与纸面 18mm padding 叠加成 30mm，改为 `margin:0` 后由纸面自带页边距；Chromium 实测打印 PDF 与预览同为 3 页、无空白尾页。三个新字段（`avatar`/`avatarCrop`/`subtitle`）均为可选，`BACKUP_SCHEMA_VERSION` 与 Dexie schema 都不变，旧 v4 备份照常导入。
