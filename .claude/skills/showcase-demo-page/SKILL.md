...
2. **视觉一律抄源项目，不要自选风格**：主题变量文件（`variables.scss` / `element-variables.scss` / `tailwind.config` / CSS 变量）里的色号、组件库皮肤、布局尺寸全部取真值写进 `:root`。只有在既无源码又无截图时，才用 `landing-anatomy.md` 末尾的风格库定方向。
3. **外壳也要复刻**：侧栏（含真实菜单树）、顶栏、页签条、面包屑一起还原，不能只做内容区——观众第一眼认的是外壳。系统有多个代表性界面（如对话页 + 主表操作台）时，用真实菜单在演示页内切换。
4. 逐项替换骨架里的占位：模块名、表格列、状态文案、按钮措辞、假数据。留一个占位没换就是交付事故。
5. 保真度默认交付 L1（骨架）+ L2（流程），L3（细节）只挑一处点睛。
...
| 熟悉该系统的人说「不像」 | 自己选了配色排版，或只复刻内容区、没复刻侧栏顶栏；列名与状态文案用了近义词 | 回源码抄主题变量与外壳尺寸、抄文案原文（身份信息除外） |
