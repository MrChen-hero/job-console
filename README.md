...
| 模块 | 能做什么 |
|---|---|
| 工作台 | 投递漏斗与进度统计；待办按日期排序，以日期块颜色区分已逾期、今天、后续，默认全部展示并支持滚动、直达投递详情；手机统计两列、待办优先；求职里程碑与快捷入口 |
...
- **运行时通道**：页面上传 .md / .html 或添加演示链接，存入浏览器并与内置内容合并。示例项目与默认分类可恢复；文档和演示页不提供单独的“重置为内置”。
